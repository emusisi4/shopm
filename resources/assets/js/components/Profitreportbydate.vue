...
<div>
     ...
     <div class="col container-fluid dblock container-fluid front-sec ml-0 px-0">    
      ...
      <has-error :form="form" field="enddate"></has-error>
                    ...
                    <option v-for='data in brancheslist' v-bind:value='data.walletno'>{{ data.id }} - {{ data.name }}</option>
                           ...
                           <td>  {{((mydataObjectinfo.datesold))}}</td>
                              <td>   <template v-if="mydataObjectinfo.branch_name">	{{mydataObjectinfo.branch_name.branchname}}</template></td>
                               <td>    <template v-if="mydataObjectinfo.product_name">	{{mydataObjectinfo.product_name.name}}</template></td>
                                       ...
                                       <td> Ugx {{formatPrice((mydataObjectinfo.unitcost))}} </td>
                                         <td> Ugx {{formatPrice((mydataObjectinfo.unitcost*mydataObjectinfo.quantity))}} </td>
                                         <td> Ugx {{formatPrice((mydataObjectinfo.unitprice))}} </td>
                                           <td> Ugx {{formatPrice((mydataObjectinfo.unitprice*mydataObjectinfo.quantity))}} </td>
                                      <td> Ugx {{formatPrice(  ((mydataObjectinfo.unitprice)) - ((mydataObjectinfo.unitcost)) )}} </td>
                                       <td> Ugx {{formatPrice(  ((mydataObjectinfo.unitprice*mydataObjectinfo.quantity)) - ((mydataObjectinfo.unitcost*mydataObjectinfo.quantity)) )}} </td>
                                    
                               <td>  
                                    <div v-if="(((mydataObjectinfo.lineprofit*mydataObjectinfo.quantity)) < 1) && (((mydataObjectinfo.lineprofit*mydataObjectinfo.quantity)) >= 0) ">
...
allowedtoaccesscomponent : null,
                form: new Form({
                                    ...
                                    description : '',
                                    amount : '',
                                    actionaid : 'profitreportbybranch',
                                    datemade : '',
                                    branch : ''
...
// Our method to GET results from a Laravel endpoint
      getResults(page = 1) {
                        axios.get('api/getproductsalesreportdetails?page=' + page)
                          .then(response => {
...
this.form.put('api/getproductsalesreportdetailsd/'+this.form.id)
  .then(()=> {
    // on success
   $('#addNew').modal('hide');
    swal.fire(
  ...
  confirmButtonText: 'Yes, delete it!'
    ...
    if(result.value){
           ...
           checkAccess(){
            ...
            loadDatarecords(){
      axios.get("api/expensesreportbywalletcomponentaccess").then(({ data }) => (this.allowedtoaccesscomponent = data));
      axios.get("api/getproductsalesreportdetails").then(({ data }) => (this.mydataObject = data));
